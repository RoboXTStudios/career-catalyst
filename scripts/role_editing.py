...
BANNED_PHRASE_REPLACEMENTS = {
    "generic clear plan": "specific operating structure",
    "clear plan": "usable operating plan",
    ...
    "built my career around": "my experience has centered on",
}
...
def rewrite_banned_voice_phrases(text: str, banned_phrases: list[str]) -> tuple[str, list[str]]:
    """Deterministically soften replaceable banned voice phrases before validation."""
    rewritten = text
    applied: list[str] = []
    for phrase in sorted({str(value).lower() for value in banned_phrases if value}, key=len, reverse=True):
        replacement = BANNED_PHRASE_REPLACEMENTS.get(phrase)
        if not replacement:
            continue
        if phrase == "clear plan":
            pattern = re.compile(r"\bclear plans?\b", re.IGNORECASE)

            def replace_clear_plan(match: re.Match[str]) -> str:
                matched = match.group(0)
                return "usable operating plans" if matched.lower().endswith("s") else replacement

            rewritten, count = pattern.subn(replace_clear_plan, rewritten)
        else:
