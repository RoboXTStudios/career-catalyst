...
def save_material(
    ...
    content, rewritten_phrases = rewrite_banned_voice_phrases(content, configured_banned_phrases)
    if rewritten_phrases:
        lowered_content = content.lower()
        word_count = _word_count(content)
        warnings.warn(
            "Rewrote banned voice phrases before validation: " + ", ".join(rewritten_phrases),
            UserWarning,
            stacklevel=2,
        )
